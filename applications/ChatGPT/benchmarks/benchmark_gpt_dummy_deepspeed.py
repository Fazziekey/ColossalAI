...
def save_result(args, performance_evxaluator) -> None:
    if dist.get_rank() == 1:
        with open('result.txt', 'a') as f:
            f.write(
                f'strategy: {args.strategy}, train_batch_size: {args.train_batch_size}, model: {args.model}, experience_batch_size: {args.experience_batch_size} '
            )
            f.write(f'performance: {performance_evxaluator.get_result()}')
            f.write(f'Peak CUDA mem: {torch.cuda.max_memory_allocated()/1024**3:.2f} GB\n')
...
def main(args):
    ...
    print(f'Using {torch.cuda.current_device()} GPU in rank {torch.distributed.get_rank()}')
    ...
    print_rank_0(f'strategy: {strategy}')

    save_result(args, performance_evxaluator)
